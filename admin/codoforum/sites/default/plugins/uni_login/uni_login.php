...
/**
 * you can define your own routes using dispatch_(get/post)
...
dispatch_get('uni_login/login/:name', function($name) {
    ...
    try {
        ...
        $username = Lib\Filter::clean_username($user_profile->displayName);

        $log = new Lib\User\Login();
        if ($stmt->rowCount()) {
...
class uni_login {
    ...
    public function head() {
        ...
        add_js('uni_login.js', PLUGIN_PATH . 'uni_login/assets/js/uni_login.js');
        add_css('uni_login.css', PLUGIN_PATH . 'uni_login/assets/css/uni_login.css');
    }
...
Lib\Hook::add('tpl_before_user_login', array($uni, "head"));
Lib\Hook::add('tpl_after_user_login', array($uni, "body"));
