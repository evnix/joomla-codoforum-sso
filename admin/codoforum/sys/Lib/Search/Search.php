...
class Search {
    ...
    public function get_matching_str($str) {
        ...
        $start = min(array_filter($first_positions));
    ...
    private function base_query() {
        ...
        $qry = 'SELECT ' . $count . ' p.post_id, p.imessage AS message,p.imessage, p.post_created,p.post_modified, '
                . 'u.id, u.username as name, u.avatar, u.rid, u.signature, c.cat_img, c.cat_alias,'
        ...
        $qry .= ' ORDER BY #SORT# #ORDER# LIMIT  ' . $this->from . ', ' . $this->num_results;
